        ...
        return (
            <section className="">
                <div className="flex flex-col items-center md:flex-row md:items-start p-4 relative">
                    <img className="w-[10rem] object-contain z-10 2xl:w-[15rem]" src={info.data?.image} alt={info.data?.title?.romaji} />
                    <div className="ml-4 z-10 text-slate-100">
                        <h1 className="text-2xl py-1 mt-1 text-center md:text-2xl md:text-start md:pr-24 lg:text-3xl xl:text-3xl ">{info.data?.title?.romaji} ({info.data?.releaseDate})</h1>
                        ...
                        <h4 className="text-xl absolute right-3 top-0 flex justify-center items-center text-teal-400 lg:text-3xl">
                            <FaStar className="text-yellow-500 text-3xl mt-1 mr-1 xl:text-4xl" />{info.data?.rating / 10}+</h4>
                    </div>
